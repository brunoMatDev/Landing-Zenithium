...
export default function Portfolio() {
  ...
  function filterProjects(featured) {
    return projects.filter(function(p) { return p.featured === featured; });
  }

  const featuredProjects = filterProjects(true);
  const otherProjects = filterProjects(false);
      ...
      {otherProjects.length > 0 && (
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {otherProjects.map(function(project, index) {
              return (
                <div 
                  key={project.title}
